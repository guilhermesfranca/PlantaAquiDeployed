    ...
    <section className="font-sans text-[#594236]">
      <div className="max-w-2xl mx-auto bg-white/70 rounded-2xl shadow-lg p-6 sm:p-8">
        <header className="mb-6 text-center">
          <h2 className="text-2xl font-semibold">🏆 Global Ranking</h2>
          <p className="text-sm text-[#96a78d]">
            Meet the leaders of this week's green missions.
          </p>
        </header>
          ...
          {ranking.map((entry, index) => {
            ...
            return (
              <li
                ...
                aria-label={`${index + 1} place: ${entry.name} with ${entry.points} points`}
              >
                <div className="flex items-center gap-4">
                  <span
                    className={`flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-full border-2 font-semibold ${
                      isFirst
                        ? "text-[#594236] border-[#d4af37] text-xl sm:text-2xl"
                        : "border-[#96a78d] text-lg"
                    }`}
                  >
                    {index + 1}
                  </span>
                  <div>
                    ...
                    <p className="text-sm text-[#96a78d]">
                      Completed missions and positive impact on the community.
                    </p>
                  </div>
                </div>
                <span
                  className={`font-semibold ${
                    isFirst
                      ? "text-lg sm:text-xl text-[#594236]"
                      : "text-base text-[#594236]"
                  }`}
                >
                  {entry.points} pts
                </span>
